...
const FP_MAP = { 0: 0, 3: 6, 1: 4, 4: 3, 2: 1 };
const IP_MAP = { 0: 0, 1: 3, 2: 6, 3: 1, 4: 4 };
 ...
 */
export function partialThreeTwoEncode(
    decoded: Buffer,
    ...
    // > Progressive
    let readOff = 0, off = 0,
        buffer = Buffer.alloc(Math.ceil(((decoded && decoded.length) + carryLength / 8) / 5) * 8);
    ...
    if (complete) {
        ...
        if (addPadding) {
            for (let i = 0; i < FP_MAP[carryLength]; ++i) {
 ...
 */
export function partialThreeTwoDecode(
    encoded: Buffer | null,
    alphabet: BaseAlphabet,
    carry: number,
    carryLength: number,
